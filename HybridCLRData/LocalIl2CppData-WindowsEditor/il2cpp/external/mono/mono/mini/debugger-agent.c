...
#ifdef RUNTIME_IL2CPP
	Il2CppCustomAttributeDataList* attrs = il2cpp_get_custom_attribute_data_list(attr_klass, cinfo, image);
	if (attrs == NULL)
...
frame_commands (int command, guint8 *p, guint8 *end, Buffer *buf)
{
	...
	case CMD_STACK_FRAME_SET_THIS: {
		...
		if (frame->de.ji->is_interp) {
			...
			set_var (mono_class_is_valuetype (frame->actual_method->klass), var, &frame->ctx, frame->de.domain, val_buf, frame->reg_locations, &tls->restore_state.ctx);
		}
		break;
	}
	default:
